...
router.get('/get-user-character', async (req, res) => {
  try {
    ...
    const chatlog = await ChatLog.find({
      userId: user._id,
      modelName: modelName,
    }).sort({ timestamp: -1 });
    ...
    if (!chatlog.length) {
      return res
        .status(404)
        .json({ message: 'No chat logs found for this model.' });
    } // if
...
router.get('/get-user', async (req, res) => {
  try {
    ...
    const chatlog = await ChatLog.find({ userId: user._id }).sort({
      timestamp: -1,
    });
    ...
    if (!chatlog.length) {
      return res
        .status(404)
        .json({ message: 'No chat logs found for this user.' });
    } // if
